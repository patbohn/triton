...
class MultiSampleGMM:
    ...
    """
    def __init__(self, fixed_means, fixed_covs, fixed_weight_ratios, 
                 n_new_components, n_samples, **kwargs):
        """
        Parameters:
        -----------
    ...
    def _initialize(self, X_list):
        ...
        for i in range(self.n_samples):
            self.weights_[i, :self.n_fixed] = self.fixed_ratios / np.sum(self.fixed_ratios)
            if self.n_new > 0:
                # Initialize new components using pooled data
                X_all = np.concatenate(X_list)
                idx = rng.choice(len(X_all), self.n_new)
                self.means_[self.n_fixed:] = X_all[idx].reshape(-1, 1)
                self.covariances_[self.n_fixed:] = np.var(X_all) * np.ones((self.n_new, 1, 1))
...
class NanoporeGMM:
    ...
    def fit_modified_samples(self, signal_list, sample_ids=None):
        ...
        for n_new in range(0, self.max_modified_components + 1):
            ...
            bic = (-2 * ll_improvement +
                   n_params * np.log(len(X_all)))
            
            logging.debug(f"n_new: {n_new}, BIC: {bic:.2f}")
            logging.debug(f"New component means: {gmm.means_[self.unmodified_gmm.n_components:]}")
    ...
    def get_modification_rates(self, signal_list: list, with_ci=False, n_bootstrap=100, ci_level=0.95):
        ...
        for i, sample_id in enumerate(self.sample_ids):
            ...
            rates[sample_id] = (
                np.sum(self.modified_gmm.weights_[i, self.unmodified_gmm.n_components:]),
                lower,
                upper
            )
    ...
    def save_model(self, filename):
        ...
        model_data = {
            ...
            'modified': None if self.modified_gmm is None else {
    ...
    def load_model(cls, filename):
        ...
        model.unmodified_gmm = GaussianMixture(n_components=model_data['unmodified']['n_components'])
        model.unmodified_gmm.means_ = np.array(model_data['unmodified']['means'])
        model.unmodified_gmm.covariances_ = np.array(model_data['unmodified']['covariances'])
        ...
        if model_data['modified'] is not None:
            ...
            model.modified_gmm = MultiSampleGMM(
                ...
                n_new_components=model_data['modified']['n_components'] - model_data['unmodified']['n_components'],
                n_samples=len(model_data['modified']['weights'])
            )
            model.modified_gmm.means_ = np.array(model_data['modified']['means'])
            model.modified_gmm.covariances_ = np.array(model_data['modified']['covariances'])
...
def fit_gmm(
    ...
    model.fit_modified_samples(
        mod_signals, 
        sample_ids=mod_names
        )
