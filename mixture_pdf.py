...
class NestedMixtureModel:
    ...
    def _log_likelihood(self, params, data):
        ...
        for x in data:
            ...
            point_likelihood = (
                (1 - modification_rate) * unmodified_pdf + 
                modification_rate * modified_pdf
            )
    ...
    def fit(self, data, initial_guess=None):
        ...
        if initial_guess is None:
            ...
            unmodified_means = np.percentile(data, 
                np.linspace(25, 75, n_u))
            modified_means = np.percentile(data, 
                np.linspace(25, 75, n_m))
        ...
        self.modified_weights = result.x[n_u*3:n_u*3+n_m] / np.sum(result.x[n_u*3:n_u*3+n_m])
        self.modified_means = result.x[n_u*3+n_m:n_u*3+n_m+n_m]
        self.modified_stds = np.abs(result.x[n_u*3+n_m+n_m:n_u*3+n_m+n_m*2])
...
def example_usage():
    ...
    model = NestedMixtureModel(
        n_unmodified_components=2, 
        n_modified_components=2
    )
