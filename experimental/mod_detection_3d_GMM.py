...
class ModificationAnalyzer:
    ...
    def fit_combined_data(
        self, sample_dict: Dict[str, pd.DataFrame], samples_per_condition: int = 1000
    ) -> None:
        """
        ...
        for sample_name, data in sample_dict.items():
            features = self.prepare_features(
                data["signal"].values, data["dwell_time"].values, data["std_dev"].values
            )
    ...
    def analyze_samples(
        self, sample_dict: Dict[str, pd.DataFrame], control_map: Dict[str, str]
    ) -> Dict[str, Dict]:
        """
        ...
        for sample_name, control_name in control_map.items():
            ...
            def nan_kstest(s1, s2):
                return -np.log10(
                    kstest_ind(s1[~np.isnan(s1)], s2[~np.isnan(s2)]).pvalue
                )
