...
class ModificationRateEstimator:
    ...
    def plot_fit(
        self,
        control_kde: stats.gaussian_kde,
        modified_kde: stats.gaussian_kde,
        x_grid: np.ndarray,
        weight: float,
        title: str = "",
    ) -> None:
        ...
        plt.plot(
            x_grid, weight * control_pdf, "r-", label=f"Control (weight={weight:.3f})"
        )
        plt.fill_between(
            x_grid,
            modified_pdf,
    ...
    def estimate_modification_rate(
        ...
        control_kdes: Optional[List[stats.gaussian_kde]] = None,
    ) -> Tuple[float, float]:
        ...
        def objective(weight: float, control_kde: stats.gaussian_kde) -> float:
            ...
            residuals = np.maximum(
                0, weight * control_pdf - modified_pdf - self.overlap_tolerance
            )
        ...
        for control_kde in control_kdes:
            res = optimize.minimize_scalar(
                lambda w: objective(w, control_kde), bounds=(0, 1), method="bounded"
            )
...
def estimate_rates_for_position(position_data: dict) -> dict:
    ...
    for sample, metrics in position_data["modified_samples"]:
        rate, uncertainty = estimator.estimate_modification_rate(
            control_signal, metrics["signal"], control_kdes
        )
